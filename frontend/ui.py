...
def _recording_screen_html(timer_text: str) -> str:
    ...
    return f"""<div style="background:#F8F6F1;padding:32px 48px;margin:0;display:flex;flex-direction:column;align-items:center;justify-content:center;"><div id="clarke-rec-dot" style="display:inline-block;width:24px;height:24px;background:#D4AF37;border-radius:50%;animation:recordPulse 2s ease-in-out infinite;margin-bottom:16px;"></div><div id="clarke-rec-label" style="font-family:Inter,sans-serif;font-size:13px;font-weight:600;color:#D4AF37;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:24px;">Recording</div><div id="clarke-rec-timer" style="font-family:'JetBrains Mono',monospace;font-size:56px;color:#1A1A2E;letter-spacing:0.05em;margin-bottom:16px;">{escape(timer_text)}</div><div id="clarke-rec-instructions" style="font-family:Inter,sans-serif;font-size:14px;color:#1A1A2E;margin-top:12px;">1. To record live audio: use the microphone below, press stop when finished, and wait for the green Audio Ready confirmation. Then click End Consultation.<br><br>2. To use the built-in demo audio:directly click End Consultation without recording.</div></div>"""
...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(theme=clarke_theme, css=Path("frontend/assets/style.css").read_text(encoding="utf-8"), title="Clarke", head=CLARKE_HEAD) as demo:
        ...
        with gr.Column(visible=False) as screen_s3:
            ...
            gr.HTML("""<div style='position:sticky; bottom:0; left:0; right:0; z-index:100;'><button id='clarke-end-consultation-btn' onclick="(function(){var btn=document.getElementById('clarke-end-consultation-btn');if(btn.classList.contains('clarke-end-btn-disabled'))return;var el=document.getElementById('hidden-end-consultation');if(!el)return;if(el.tagName==='BUTTON'){el.click();}else{var b=el.querySelector('button');if(b)b.click();}console.log('Clarke: End Consultation clicked');})()" class='clarke-end-btn-ready' style='display:block; width:100%; padding:18px 0; border:none; font-family:Inter,sans-serif; font-weight:700; font-size:16px; letter-spacing:0.5px; transition:all 0.3s ease;'>End Consultation</button></div><script>(function(){var lastState='idle';setInterval(function(){var ai=document.getElementById('clarke-audio-input');var btn=document.getElementById('clarke-end-consultation-btn');var dot=document.getElementById('clarke-rec-dot');var lbl=document.getElementById('clarke-rec-label');var tmr=document.getElementById('clarke-rec-timer');var inst=document.getElementById('clarke-rec-instructions');if(!ai||!btn)return;var btns=ai.querySelectorAll('button');var isRecording=false;btns.forEach(function(b){var t=b.textContent.trim();if(t==='Stop'||t==='Pause')isRecording=true;});if(isRecording&&lastState!=='recording'){lastState='recording';btn.className='clarke-end-btn-disabled';btn.textContent='Recording...';}else if(!isRecording&&lastState==='recording'){lastState='processing';btn.className='clarke-end-btn-disabled';btn.textContent='Processing audio...';if(dot){dot.style.background='#D4AF37';dot.style.animation='clarkeSpin 1s linear infinite';dot.style.borderRadius='4px';}if(lbl){lbl.style.color='#D4AF37';lbl.textContent='PROCESSING AUDIO...';}if(tmr)tmr.style.display='none';if(inst)inst.textContent='Saving your recording...';setTimeout(function(){if(lastState==='processing'){lastState='ready';btn.className='clarke-end-btn-ready';btn.textContent='End Consultation';if(dot){dot.style.background='#4CAF50';dot.style.animation='none';dot.style.borderRadius='50%';dot.style.boxShadow='0 0 16px rgba(76,175,80,0.4)';}if(lbl){lbl.style.color='#4CAF50';lbl.textContent='AUDIO READY';}if(inst)inst.textContent='Recording saved. Click End Consultation to generate your document.';}},2000);}},300);})();</script>""")

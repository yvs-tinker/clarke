...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(theme=clarke_theme, css=Path("frontend/assets/style.css").read_text(encoding="utf-8"), title="Clarke", head=CLARKE_HEAD) as demo:
        ...
        with gr.Column(visible=False) as screen_s6:
            ...
            gr.HTML("""<div style='display:flex;gap:12px;margin-top:24px;justify-content:center;'><button onclick=\"(function(){var el=document.getElementById('signed-letter-text');var text='';if(el){text=el.innerText||el.textContent;}if(!text){document.querySelectorAll('textarea').forEach(function(t){if(t.value&&t.value.length>50)text=t.value;});}if(!text){alert('No letter text found');return;}try{navigator.clipboard.writeText(text.trim()).then(function(){alert('Copied to clipboard!');});}catch(e){var ta=document.createElement('textarea');ta.value=text.trim();document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);alert('Copied to clipboard!');}})()\" style='background:transparent; color:#1A1A2E; border:2px solid #D4AF37; padding:12px 24px; border-radius:8px; font-family:Inter,sans-serif; font-weight:600; font-size:14px; cursor:pointer; transition:all 0.3s ease;' onmouseover=\"this.style.background='rgba(212,175,55,0.1)';this.style.boxShadow='0 0 12px rgba(212,175,55,0.3)';this.style.transform='translateY(-2px)'\" onmouseout=\"this.style.background='transparent';this.style.boxShadow='none';this.style.transform='translateY(0)'\">📋 Copy to Clipboard</button><button onclick=\"(function(){console.log('Clarke: PDF generation started');var el=document.getElementById('signed-letter-text');var text='';if(el){text=el.innerText||el.textContent;}if(!text){document.querySelectorAll('textarea').forEach(function(t){if(t.value&&t.value.length>50)text=t.value;});}if(!text){alert('No letter text found');return;}text=text.trim();if(typeof window.jspdf==='undefined'){alert('PDF library still loading. Please try again in a moment.');return;}var jsPDF=window.jspdf.jsPDF;var doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});var pageW=210;var pageH=297;var mL=20;var mR=20;var mT=25;var mB=25;var usableW=pageW-mL-mR;var y=mT;var headings=['History of Presenting Complaint','Examination','Investigations','Assessment','Plan'];function isHeading(line){for(var i=0;i<headings.length;i++){if(line.trim()===headings[i])return true;}return false;}function addPage(){doc.addPage();y=mT;drawHeaderLine();}function drawHeaderLine(){doc.setDrawColor(212,175,55);doc.setLineWidth(0.8);doc.line(mL,mT-5,pageW-mR,mT-5);doc.setFontSize(12);doc.setTextColor(150,150,150);doc.setFont('helvetica','normal');doc.text('Clarke NHS Trust — Confidential',mL,mT-7);doc.text('Page '+doc.getNumberOfPages(),pageW-mR,mT-7,{align:'right'});y=mT+2;}function checkSpace(needed){if(y+needed>pageH-mB){addPage();}}drawHeaderLine();doc.setFontSize(12);doc.setTextColor(120,120,120);doc.setFont('helvetica','normal');doc.text('Clarke NHS Trust',pageW-mR,y,{align:'right'});y+=3.5;doc.text('General Practice Department',pageW-mR,y,{align:'right'});y+=3.5;doc.text('University Hospital London',pageW-mR,y,{align:'right'});y+=8;doc.setDrawColor(212,175,55);doc.setLineWidth(0.5);doc.line(mL,y,pageW-mR,y);y+=8;var lines=text.split('\\n');var i=0;while(i<lines.length){var line=lines[i].trim();if(!line){y+=3;i++;continue;}if(i<6&&!isHeading(line)){doc.setFontSize(12);doc.setTextColor(30,30,30);doc.setFont('helvetica','normal');if(line.match(/^Dear Dr/)){y+=3;doc.setFont('helvetica','normal');}if(line.match(/^Re:/)){doc.setFont('helvetica','bold');doc.setFontSize(13);}var wrapped=doc.splitTextToSize(line,usableW);checkSpace(wrapped.length*6);doc.text(wrapped,mL,y);y+=wrapped.length*6;if(line.match(/^Re:/)){doc.setFont('helvetica','normal');doc.setFontSize(12);y+=2;}i++;continue;}if(isHeading(line)){y+=5;checkSpace(15);doc.setFontSize(14);doc.setFont('helvetica','bold');doc.setTextColor(26,26,46);doc.text(line,mL,y);y+=2;doc.setDrawColor(212,175,55);doc.setLineWidth(0.3);doc.line(mL,y,mL+doc.getTextWidth(line),y);y+=5;doc.setFontSize(12);doc.setFont('helvetica','normal');doc.setTextColor(30,30,30);i++;continue;}if(line.match(/^Warm regards/)){y+=8;checkSpace(30);doc.setFont('helvetica','normal');doc.setFontSize(12);doc.text('Warm regards,',mL,y);y+=12;doc.setFont('helvetica','bold');doc.setFontSize(13);i++;while(i<lines.length){var sl=lines[i].trim();if(sl){doc.text(sl,mL,y);y+=5;}i++;}continue;}if(line.match(/^\\d+\\./)){var wrapped2=doc.splitTextToSize(line,usableW-5);checkSpace(wrapped2.length*6);doc.text(wrapped2,mL+3,y);y+=wrapped2.length*6;i++;continue;}var wrapped3=doc.splitTextToSize(line,usableW);checkSpace(wrapped3.length*6);doc.text(wrapped3,mL,y);y+=wrapped3.length*6;i++;}doc.save('clinic_letter.pdf');console.log('Clarke: PDF generated and downloaded');})()\" style='background:transparent; color:#1A1A2E; border:2px solid #D4AF37; padding:12px 24px; border-radius:8px; font-family:Inter,sans-serif; font-weight:600; font-size:14px; cursor:pointer; transition:all 0.3s ease;' onmouseover=\"this.style.background='rgba(212,175,55,0.1)';this.style.boxShadow='0 0 12px rgba(212,175,55,0.3)';this.style.transform='translateY(-2px)'\" onmouseout=\"this.style.background='transparent';this.style.boxShadow='none';this.style.transform='translateY(0)'\">📑 Download as PDF</button><button onclick=\"(function(){console.log('Clarke: Download clicked');var el=document.getElementById('signed-letter-text');var text='';if(el){text=el.innerText||el.textContent;}if(!text){document.querySelectorAll('textarea').forEach(function(t){if(t.value&&t.value.length>50)text=t.value;});}if(!text){alert('No letter text found');return;}var a=document.createElement('a');a.href='data:text/plain;charset=utf-8,'+encodeURIComponent(text.trim());a.download='clinic_letter.txt';a.style.display='none';document.body.appendChild(a);a.click();document.body.removeChild(a);console.log('Clarke: Download complete via data URI');})()\" style='background:transparent; color:#1A1A2E; border:2px solid #D4AF37; padding:12px 24px; border-radius:8px; font-family:Inter,sans-serif; font-weight:600; font-size:14px; cursor:pointer; transition:all 0.3s ease;' onmouseover=\"this.style.background='rgba(212,175,55,0.1)';this.style.boxShadow='0 0 12px rgba(212,175,55,0.3)';this.style.transform='translateY(-2px)'\" onmouseout=\"this.style.background='transparent';this.style.boxShadow='none';this.style.transform='translateY(0)'\">📄 Download as Text</button></div>""")
